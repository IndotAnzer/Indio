...
from app.adapters.netease.radio import (
    primary_artist_key,
...
CATALOG: list[Track] = [
    Track(
        ...
        artwork_url=None,
        platform_url=None,
        playback_source=PlaybackSource.FALLBACK,
    ),
    Track(
...
def _score_fallback(track: Track, query: str) -> int:
    normalized = query.lower()
    text = " ".join([track.title, track.artist, track.album, track.mood]).lower()
    score = 0
    if normalized and normalized in text:
        score += 8
    for word in ("focus", "专注", "工作"):
        if word in normalized and track.mood in {"focus", "work"}:
            score += 4
    for word in ("quiet", "安静", "轻"):
        if word in normalized and track.mood in {"quiet", "morning"}:
            score += 4
    if ("morning" in normalized or "早" in normalized) and track.mood == "morning":
        score += 4
    if any(word in normalized for word in ("evening", "晚", "夜")) and track.mood == "evening":
        score += 4
    return score
...
class NeteaseMusicAdapter:
    ...
    def get_status(self) -> MusicStatus:
        ...
        return MusicStatus(
            ...
            provider="netease-api-enhanced" if configured else "fallback",
            ...
            detail=(
                ...
                else "api-enhanced 已配置，但还没有绑定网易云账号，当前不会走公开搜歌，只会回退到本地占位曲库。"
                if configured
                else "未配置 api-enhanced 服务，当前会退回到本地 fallback 曲库。"
            ),
        )
    ...
    async def search(self, query: str) -> list[Track]:
        fallback = self._search_fallback(query)
        ...
        if self.state.get_music_auth() and library:
            return await self._pick_playable_tracks(
                radio_sort_library_tracks(library, query or "late night radio", recent),
                6,
                diversify_artists=True,
            )
        if not self.config.netease_api_base_url or not self.state.get_music_auth():
            return fallback
        try:
            results = await self._search_catalog(query)
    ...
    async def get_recommendations(self, mood: str | None = None, query: str | None = None) -> list[Track]:
        ...
        if self.config.netease_api_base_url and self.state.get_music_auth():
            try:
                ...
                pass
        exact = [track for track in CATALOG if track.mood.lower() == hint]
        if exact:
            return exact
        if "quiet" in hint or "soft" in hint:
            return [track for track in CATALOG if track.mood in {"quiet", "morning"}]
        if "morning" in hint:
            return [track for track in CATALOG if track.mood in {"morning", "focus"}]
        return [track for track in CATALOG if track.mood in {"focus", "work", "evening"}]
    ...
    async def resolve_playable_source(self, track: Track) -> Track:
        if not track.netease_id or not self.config.netease_api_base_url:
            track.playback_source = PlaybackSource.NETEASE if track.stream_url else PlaybackSource.FALLBACK
            return track
        if track.stream_url:
        ...
        try:
            ...
            return track.model_copy(update={
                ...
                "playback_source": PlaybackSource.NETEASE if stream_url else PlaybackSource.FALLBACK,
            })
        except Exception:
            return track.model_copy(update={"playback_source": PlaybackSource.FALLBACK})
    ...
    def _search_fallback(self, query: str) -> list[Track]:
        ranked = sorted(((track, _score_fallback(track, query)) for track in CATALOG), key=lambda item: item[1], reverse=True)
        results = [track for track, score in ranked if score > 0]
        return results or [track for track in CATALOG if track.mood == "focus"]
